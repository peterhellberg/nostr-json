// Package npub is partially based on code found in
// the nip19 package in github.com/nbd-wtf/go-nostr
package npub
...
// Decode bech32 encoded npub string into its correspoding hex string
func Decode(s string) (string, error) {
	prefix, data, err := decode(s)
	if err != nil {
		return "", err
	}

	if prefix != "npub" {
		return "", fmt.Errorf("prefix is not npub")
	}

	bits, err := convertBits(data, 5, 8, false)
	if err != nil {
		return "", fmt.Errorf("failed translating data into 8 bits: %s", err.Error())
	}

	if len(bits) < 32 {
		return "", fmt.Errorf("data is less than 32 bytes (%d)", len(bits))
	}

	return hex.EncodeToString(bits[0:32]), nil
}
...
func decode(s string) (string, []byte, error) {
	...
	for i := 0; i < len(s); i++ {
		if s[i] < 33 || s[i] > 126 {
			return "", nil, fmt.Errorf("invalid character in string: '%c'", s[i])
		}
	}
